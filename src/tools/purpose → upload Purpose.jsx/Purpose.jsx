...
const STEP_NAMES    = ["1. Who FBaM serves","2. Positioning","3. Purpose","4. Mission","5. Distinctiveness","6. VRIN test","7. Disappearance","8. WHY/HOW/WHAT","9. Finalise"];
  ...
  return (
    <div className="sl">
      <div className="sl-entry">
        ...
        <div className="sl-brand-sub">Purpose Tool</div>
...
function ParticipantView({ name, tick, onLogout }) {
  ...
  const advance = () => { if (displayStep < 9) setDisplayStep(s => s + 1); };
  ...
  return (
    <div className="sl-shell">
      <div className="sl-header">
        <div className="sl-header-title">Purpose Tool</div>
      ...
      {displayStep === 1 && <PurposeStep8 pData={pd} confirmed={pd.step9Confirmed} onConfirm={advance} onBack={onLogout} />}
      {displayStep === 2 && <PurposeStep9 pData={pd} confirmed={pd.step10Confirmed} onConfirm={(t) => { pSave(name, { purposeTensions: t, step10Confirmed: true }); advance(); }} onBack={() => go(1)} />}
      {displayStep === 3 && (
        <PurposeOptionsStep
...
- Number each 1-4, one per line, no other commentary`)}
          confirmLabel="Confirm → Step 4: Mission"
          onConfirm={advance} onBack={() => go(2)}
        />
      )}
      {displayStep === 4 && (
        <PurposeOptionsStep
...
- Number each 1-4, one per line, no other commentary`)}
          confirmLabel="Confirm → Step 5: Distinctiveness"
          onConfirm={advance} onBack={() => go(3)}
        />
      )}
      {displayStep === 5 && (
        <PurposeOptionsStep
...
- Number each 1-4, one per line, no other commentary`)}
          confirmLabel="Confirm → Step 6: VRIN test"
          onConfirm={advance} onBack={() => go(4)}
        />
      )}
      {displayStep === 6 && <PurposeStep13 pData={pd} confirmed={pd.step14Confirmed} onConfirm={advance} onBack={() => go(5)} />}
      {displayStep === 7 && (
        <PurposeOptionsStep
...
- Each option is 1 sentence, number each 1-4, one per line, no other commentary`)}
          confirmLabel="Confirm → Step 8: WHY/HOW/WHAT"
          onConfirm={advance} onBack={() => go(6)}
        />
      )}
      {displayStep === 8 && <PurposeStep15 pData={pd} confirmed={pd.step16Confirmed} onConfirm={advance} onBack={() => go(7)} />}
      {displayStep === 9 && <Step20Finalise pData={pd} onBack={() => go(8)} />}
    </div>
  );
}
    ...
    <div className="sl-shell">
      ...
      <div className="sl-fac">
        ...
        <div className="sl-adv-bar">
          ...
          <button className="sl-btn" style={{ fontSize: 12, padding: "8px 16px" }} onClick={advanceAll} disabled={STORE.step >= 9}>
            Advance all → Step {Math.min(STORE.step + 1, 9)}
          </button>
...
export default function PurposeTool() {
